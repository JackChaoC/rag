...
如果 Qdrant Upsert 成功但 PostgreSQL 状态切换失败，消息必须重试；相同 Chunk ID 的重复 Upsert 覆盖原 Point。`DocumentIndexer` 和业务 Handler 遇到异常时只允许抛出异常，不得直接把 Document 标记为 `failed`。消费层确认仍有重试机会时，Ingest/Reindex 必须从 `indexing` 恢复为 `pending` 并记录本次异常，Delete 保持 `deleting`，随后发布 Retry 消息；只有重试耗尽后才允许 `FailureHandler` 尽力删除目标版本所有 Point、把 Document 置为 `failed` 并记录包含异常类型的 `last_error`。不得将旧版本重新标记为新版本。

同一 Document 进入 `deleting` 后不得接受新的 Ingest 或 Reindex，除非后续规范显式定义恢复流程。

验收条件：对每种 operation 重复投递至少两次，数据库版本、有效 Chunk 数量和 Qdrant Point 数量不增加；乱序旧版本不会覆盖新版本；可恢复异常发生后状态为 `pending` 或 `deleting` 而不是 `failed`，重试耗尽后才进入 `failed`；任一步骤注入失败后重试能够收敛到唯一成功或失败终态。
